...
function get_qrysm_version() {
    if [[ -n ${USE_QRYSM_VERSION:-} ]]; then
        ...
        qrysm_version=$(curl -f -s https://api.github.com/repos/rgeraldes24/prysm/releases/latest -s | jq .name -r) || (color "31" "Starting qrysm requires an internet connection. If you are being blocked by your antivirus, you can download the beacon chain and validator executables from our releases page on Github here https://github.com/rgeraldes24/prysm/releases/" && exit 1)
...
if [[ $1 == beacon-chain ]]; then
    if [[ ! -x $BEACON_CHAIN_REAL ]]; then
        ...
        res=$(curl -w '%{http_code}\n' -f -L "https://github.com/rgeraldes24/prysm/releases/download/{qrysm_version}/${file}"  -o "$BEACON_CHAIN_REAL" | ( grep 404 || true ) )
        if [[ $res == 404 ]];then
        ...
        curl --silent -L "https://github.com/rgeraldes24/prysm/releases/download/{qrysm_version}/${file}.sha256" -o "${wrapper_dir}/${file}.sha256"
        curl --silent -L "https://github.com/rgeraldes24/prysm/releases/download/{qrysm_version}/${file}.sig" -o "${wrapper_dir}/${file}.sig"
...
if [[ $1 == validator ]]; then
    if [[ ! -x $VALIDATOR_REAL ]]; then
        ...
        res=$(curl -w '%{http_code}\n' -f -L "https://github.com/rgeraldes24/prysm/releases/download/{qrysm_version}/${file}" -o "$VALIDATOR_REAL" | ( grep 404 || true ) )
        if [[ $res == 404 ]];then
...
if [[ $1 == client-stats ]]; then
    if [[ ! -x $CLIENT_STATS_REAL ]]; then
        ...
        res=$(curl -w '%{http_code}\n' -f -L "https://github.com/rgeraldes24/prysm/releases/download/{qrysm_version}/${file}" -o "$CLIENT_STATS_REAL" | ( grep 404 || true ) )
        if [[ $res == 404 ]];then
